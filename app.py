...
def load_scaler():
    return load(Path(__file__).parent / 'LSTM_Model' / 'scaler.joblib')
...
class Controller(ViktorController):
    ...
    def get_plotly_and_data_view(self, params, **kwargs):
        ...
        fig.add_trace(go.Scatter(
            x=settlement_x,
            y= -500*pred.item()*0.01 * pi * params.section1.D * params.section1.D /(4 * i * np.sqrt(2*pi)) * np.exp(-settlement_x*settlement_x/(2*i*i)),
            mode='lines',
            line=dict(color='blue', width=4,dash='dash'),
            name='Settlement profile (Increased by a factor of 500)',
            hoverinfo = 'none'  # Disable tooltips
        ))
        x_min = -15
        x_max = 15
